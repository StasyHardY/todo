...
async function start () {
  try {
    await mongoose.connect('mongodb+srv://test:test@cluster0.vspafrw.mongodb.net/todos?retryWrites=true&w=majority', {useNewUrlParser:true, useUnifiedTopology:true});
...
app.patch("/todos/:id", async (req, res) => {
  const result = await Todo.findByIdAndUpdate(req.params.id, {text:req.body.text})
	res.json(result);
})
